...
st.title("📦 SWISS Production Scheduler")
...
if uploaded_file:
    ...
    st.sidebar.title("⚙️ Speed Override by Pack Size (ml)")
    ...
    try:
        ...
        MAX_TIME = 990  # 13.5 hrs production and 3 hrs initial setup
        CHANGEOVER = 180
        fit_batches = []
        too_big_batches = []
        ...
        if too_big_batches:
            st.warning("⚠️ Some batches were too big to fit and skipped:")
            for b in too_big_batches:
                st.text(f"❌ {b['Product']} — {b['Duration']:.2f} min")
        ...
        st.download_button("📥 Download Schedule", output, file_name="Schedule.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

        st.markdown(f"📦 **Total Demand:** `{total_demand:,}` units")
        st.markdown(f"✅ **Scheduled Units:** `{int(schedule_df['Units Produced'].sum()):,}`")
        st.markdown(f"📅 **Days Used:** `{schedule_df['Date'].nunique()}`")
